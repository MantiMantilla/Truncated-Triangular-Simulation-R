...
> Let $X\sim f(x) = \frac{e^{-\frac{1}{2}\left( \frac{x-\mu}{\sigma}\right)^{2}}}{\sigma \sqrt{2\pi}}$
...
> Let $X\sim Norm(\mu, \sigma^{2})$ and $Y\sim Triangular(L,U,M)$
...
$$
  E(X) = \mu = \int_{-\infty}^{\infty}xf(x)dx = \int_{L}^{U}xf(x)dx = \frac{a+b+c}{3}
$$
...
$$
  \text{Var(X)} = \int_{-\infty}^{\infty}x^{2}f(x)dx - \mu^{2} = \int_{L}^{U}x^{2}f(x)dx - \mu^{2} = \frac{a^{2} + b^{2} + c^{2} - ab - ac - bc}{18}
$$
